...
public class Sentence
{
	...
	protected List<Word> Tokens = new ArrayList<Word>();
	protected Model DataModel = new Model();
	...
	public String toWordIndexString()
	{
		...
		for (Word word : Tokens)
		{
			sb.append(word.toWordIndexString());
			sb.append(" ");
		}
		return sb.toString();
	}
	
	public int getWordId(String label) {
		String[] labSplit = label.split(" ");
		for (Word word : Tokens)
		{
			if (word.getLemmatizedName().equals(labSplit[0]) == true) {
				return word.getId();
			}
		}
		return -1;
