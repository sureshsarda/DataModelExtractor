...
public class Entity extends Type implements Cloneable
{
	...
	public boolean equals(Entity entity)
	{
		if (this.lemmName.equals(entity.lemmName))
		{
			...
			{
				// FIXME Not considered the fact that same entities can be in
				// different order.
	...
	{
		Iterator<Attribute> attributeItr = this.Attributes.iterator();

		while (attributeItr.hasNext())
		{
			Attribute attrib = attributeItr.next();

			// Compare Lemm-names if required
			if (attrib.name.equals(name) == true)
			{
				attributeItr.remove();
				return true;
			}
